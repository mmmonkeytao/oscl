% Category and instance recognition using the depth channel
...
for i = 1:length(type)
    ...
    fea_first.maxsize = 250;
    fea_first.minsize = 50;
    fea_first.savedir = [saveroot '/rgbdeval_hmp_fea_' num2str(dic_first.patchsize) 'x' num2str(dic_first.patchsize) '_' fea_first.type];
    ...
    if ~length(rgbdfeapath)
       ...
       dic_first.dic = dic;
       save(['./rgbdeval_dic_' num2str(dic_first.patchsize) 'x' num2str(dic_first.patchsize) '_' fea_first.type '.mat'],'dic');
...
save -v7.3 rgbdfea_depth_second.mat rgbdfea rgbdclabel rgbdilabel rgbdvlabel;

load testinstance_eval;
category = 1;
if category
   trail = 10;
   for i = 1:trail
       ...
       save(['./rgbdeval_depth_acc_c_' num2str(dic_first.patchsize) 'x' num2str(dic_first.patchsize) '_second.mat'],'acc_c', 'predictlabel', 'testlabel');
...
if instance
   ...
   save(['./rgbdeval_depth_acc_i_' num2str(dic_first.patchsize) 'x' num2str(dic_first.patchsize) '_second.mat'],'acc_i', 'predictlabel', 'testlabel');
